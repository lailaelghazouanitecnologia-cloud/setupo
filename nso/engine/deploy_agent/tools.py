...
def create_tools(ctx: DeployContext) -> list[tuple]:
    ...
    async def list_ai_apps() -> str:
        """List available AI apps. Each app is an interactive pipeline with stages and templates. The agent fills in templates using project context and user input."""
        conn = await db.get_db()
        cursor = await conn.execute(
            "SELECT slug, name, description, category, input_schema, example_input, example_output, pricing, pipeline_stages, templates "
            "FROM ai_apps WHERE published = 1 ORDER BY total_runs DESC",
        )
        apps = []
        for row in await cursor.fetchall():
            app = dict(row)
            for field in ("input_schema", "example_input", "example_output", "pipeline_stages", "templates"):
    ...
    async def start_ai_app(app_slug: str, output_folder: str = "assets") -> str:
        """Start an interactive session with an AI app. Returns the app's pipeline stages, templates, and a session_id.

        The app defines stages (like 'collect_info', 'generate', 'review') and templates
        with fields to fill. You should:
        1. Read the stages and templates
        2. Fill templates using project context (memory) and by asking the user
        3. Call advance_ai_app to move through stages
        4. The final stage produces the output (saved to assets/)

        Example: start_ai_app("landing-page-builder")
        """
        ...
        stages = app.get("pipeline_stages", "[]")
        ...
        # If no stages defined, create a simple single-stage pipeline
        if not stages:
            stages = [
                {"id": "input", "name": "Input", "description": "Provide your inputs"},
                {"id": "generate", "name": "Generate", "description": "AI processes your request"},
            ]

        # Create session
        import secrets as token_gen
        from datetime import datetime, timezone
        session_id = f"ais_{token_gen.token_hex(8)}"
        now = datetime.now(timezone.utc).isoformat()

        first_stage = stages[0]["id"] if stages else "input"

        await db.insert("ai_app_sessions", {
            "id": session_id,
            "app_id": app["id"],
            "project_id": ctx.project_id,
            "user_id": ctx.user_id,
        ...
        from nso.engine.addons.ai_apps_service import gather_memory
        ...
        return json.dumps({
            ...
            "current_stage": first_stage,
            "memory": memory,
            "hint": "Fill the template for the current stage, then call advance_ai_app with the data",
        })

    async def advance_ai_app(session_id: str, stage_data: str, instance_id: str = "") -> str:
        """Advance an AI app session to the next stage. Pass the filled template data for the current stage as JSON.

        The app processes the data:
        - If there are more stages, returns the next template to fill
        - If it's the final stage, calls the AI model and returns results
        - Output files are saved to the output folder on the user's instance

        Example: advance_ai_app("ais_abc123", '{"company_name": "CafeX", "style": "modern", "colors": ["brown", "cream"]}')
        """
        session = await db.fetch_one("ai_app_sessions", id=session_id, project_id=ctx.project_id)
        if not session:
            return json.dumps({"error": "Session not found"})
        if session.get("status") != "active":
            return json.dumps({"error": f"Session is {session.get('status')}, not active"})

        app = await db.fetch_one("ai_apps", id=session["app_id"])
        if not app:
            return json.dumps({"error": "App not found"})

        # Parse stage data
        try:
            new_data = json.loads(stage_data) if isinstance(stage_data, str) else stage_data
        except json.JSONDecodeError:
            return json.dumps({"error": "Invalid JSON for stage_data"})

        # Merge into collected data
        ...
        if isinstance(collected, str):
            try:
                ...
                collected = {}
        collected[session.get("current_stage", "unknown")] = new_data
        ...
        is_final = current_idx >= len(stage_ids) - 1
        ...
        if not is_final:
            ...
            next_stage_info = stages[current_idx + 1]

            await db.update("ai_app_sessions", session_id, {
            ...
            if isinstance(templates, str):
                try:
                    ...
                    templates = {}
            next_template = templates.get(next_stage, {})
            ...
            return json.dumps({
                ...
                "is_final": current_idx + 1 >= len(stage_ids) - 1,
                "hint": "Fill this template, then call advance_ai_app again" if not (current_idx + 1 >= len(stage_ids) - 1) else "This is the final stage — fill and submit to generate output",
            })

        # ── FINAL STAGE: Execute the AI model ──
        collected[current] = new_data
        all_inputs = {}
        for stage_key, stage_val in collected.items():
            if isinstance(stage_val, dict):
                all_inputs.update(stage_val)
            else:
                all_inputs[stage_key] = stage_val

        # Gather memory
        from nso.engine.addons.ai_apps_service import gather_memory
        memory = await gather_memory(ctx.project_id, app["id"], db)
        output_folder = session.get("output_folder", "assets")
        ...
        # Execute via agent
        result = await _execute_on_agent_or_direct(
            ctx, app, all_inputs, system_prompt, memory, run_id,
            output_folder, target_instance, timeout,
        )
    ...
    return [
        ...
        (list_ai_apps, "list_ai_apps", "List available AI apps — interactive pipelines with stages and templates"),
        (start_ai_app, "start_ai_app", "Start an interactive session with an AI app — returns stages and templates to fill"),
        (advance_ai_app, "advance_ai_app", "Advance an AI app session to the next stage with filled template data"),
    ]


async def _execute_on_agent_or_direct(
    ctx: DeployContext, app: dict, inputs: dict, system_prompt: str,
    memory: dict, run_id: str, output_folder: str,
    instance_id: str, timeout: int,
) -> dict:
    """Execute AI model call via VPS agent or direct fallback."""
    if instance_id:
        inst = await db.fetch_one("instances", id=instance_id)
        if inst and inst.get("ip"):
            ...
            try:
                ...
                return resp.json() if resp.status_code == 200 else {"ok": False, "error": f"Agent HTTP {resp.status_code}"}
            except Exception as exc:
    ...
    payload = build_baseten_payload(inputs, system_prompt, memory=memory)
    return await call_baseten_model(
        api_url=app.get("baseten_api_url", ""),
        api_key=app.get("baseten_api_key", ""),
        model_id=app.get("baseten_model_id", ""),
        payload=payload,
